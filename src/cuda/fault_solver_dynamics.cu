...
__global__  void update_displ_on_fault_cuda(
...
/*    Displ[3*iglob1] += dt * Veloc[3*iglob1];
    Displ[3*iglob2] += dt * Veloc[3*iglob2];
    Displ[3*iglob1 + 1] += dt * Veloc[3*iglob1 + 1];
    Displ[3*iglob2 + 1] += dt * Veloc[3*iglob2 + 1];
    Displ[3*iglob1 + 2] += dt * Veloc[3*iglob1 + 2];
    Displ[3*iglob2 + 2] += dt * Veloc[3*iglob2 + 2];*/
	Displ[3*iglob1] += 1.0;
	Displ[3*iglob2] += -1.0;
...
__global__  void set_fault_mask_cuda(
    int* d_mask, /*mesh quantities*/
    int* d_maskax,
    int NGLOB_AB,
    int* ibulk1,
    int* ibulk2,
...

{
    ...
    iglob1 = ibulk1[tx]-1; /*since Ture is fortran is -1 numerically,we follow this convention*/
    iglob2 = ibulk2[tx]-1;
    ...
    d_maskax[3*iglob1] = 0;
    d_maskax[3*iglob2] = 0;
    d_maskax[3*iglob1 + 1] = 0;
    d_maskax[3*iglob2 + 1] = 0;
    d_maskax[3*iglob1 + 2] = 0;
    d_maskax[3*iglob2 + 2] = 0;
...
__global__  void set_mesh_mask_cuda(
    int* d_mask, /*mesh quantities*/
    int* d_maskax,
    int NGLOB_AB3,
    int myrank)
...
extern "C"
void FC_FUNC_(update_fault_displ_static,
		      UPDATE_FAULT_DISPL_STATIC)(long* Mesh_pointer,
										 long* Fault_pointer,
										 realw* dt,
										 int* myrank,
										 int* it)
{
    ...
    for(int ifault = 0; ifault < (Fsolver->Nbfaults); ifault++)
    {
        ...
        if(Flt->NGLOB_AB>0)
        {
			...
			update_displ_on_fault_cuda<<<num_of_block,128>>>(
					mp->d_displ,
					mp->d_veloc,
					Flt->NGLOB_AB,
					Flt->ibulk1,
...
extern "C"
void FC_FUNC_(set_fault_mask,
		      SET_FAULT_MASK)(long* Mesh_pointer,
							  long* Fault_pointer,
							  int* myrank)
{
    Fault_solver_dynamics* Fsolver = (Fault_solver_dynamics*)(*Fault_pointer);
    ...
    Mesh*  mp = (Mesh*)(*Mesh_pointer);
    int num_of_block;
	num_of_block = (int)(mp->NGLOB_AB*3/1024) + 1;
	set_mesh_mask_cuda<<<num_of_block,1024>>>(mp->d_maskx, mp->d_maskax, mp->NGLOB_AB*3, *myrank);
    for(int ifault = 0; ifault < (Fsolver->Nbfaults); ifault++)
    {
        Flt = &(Fsolver->faults[ifault]);
        Rsf_type* rsf = &(Flt->rsf);
        ...
        {
            num_of_block = (int) (Flt->NGLOB_AB/128)+1;
	/*	    set_fault_mask_cuda<<<num_of_block,128>>>(
					mp->d_maskx,
					mp->d_maskax,
					Flt->NGLOB_AB,
					Flt->ibulk1,
...

{
    ...
    for(int ifault = 0; ifault < (Fsolver->Nbfaults); ifault++)
    {
        ...
        if(Flt->NGLOB_AB>0)
        {
            ...
            if(true) // this is dirty implementation
            {
