...
program jfdkfd
  ...
  integer, parameter :: NX_ORI_HAUKSSON = 27, NY_ORI_HAUKSSON = 41

  double precision, dimension(NX_ORI_HAUKSSON,NY_ORI_HAUKSSON) :: rlat_ori,rlon_ori
  double precision, dimension(18,NX_ORI_HAUKSSON,NY_ORI_HAUKSSON) :: value_ori

  integer i,j,iloop,jloop,ipoin,k
  double precision xi,eta
  double precision rx1,ry1,v1(18),rx2,ry2,v2(18),rx3,ry3,v3(18),rx4,ry4,v4(18)
  double precision rx,ry,val(18),utm_x,utm_y

  ! read original titled non-evenly spaced Hauksson grid
  do j=1,NY_ORI_HAUKSSON
     do i=1,NX_ORI_HAUKSSON
        read(*,*) rlon_ori(i,j),rlat_ori(i,j),(value_ori(k,i,j),k=1,18)
     enddo
  enddo
